...
import rawNoticesData from '../data/notices.json';
...
export const DEFAULT_NOTICES: Notice[] = (rawNoticesData as any[]).map(n => ({
  id: n.id,
  title: n.title,
  content: n.content,
  category: n.category || '양성화안내',
...
// Sync notices array to backend files (src/data/notices.json, public/notices.json, index.html)
export async function syncNoticesToServerAndFiles(notices: Notice[]): Promise<{ success: boolean; count?: number; message?: string }> {
  try {
    const formatted = notices.map(n => ({
      ...
      isPinned: !!n.isPinned,
      imageUrl: n.imageUrl || '',
      createdAt: n.createdAt ? (typeof n.createdAt === 'string' ? n.createdAt : (n.createdAt.toISOString ? n.createdAt.toISOString() : new Date(n.createdAt).toISOString())) : new Date().toISOString(),
      updatedAt: n.updatedAt ? (typeof n.updatedAt === 'string' ? n.updatedAt : (n.updatedAt.toISOString ? n.updatedAt.toISOString() : new Date(n.updatedAt).toISOString())) : new Date().toISOString(),
    }));
    ...
    const res = await fetch('/api/sync-notices', {
      ...
      body: JSON.stringify({ notices: formatted })
    });

    if (res.ok) {
      const json = await res.json();
      return { success: true, count: formatted.length, message: json.message };
    }
  } catch (err) {
    console.warn('Backend file sync error (running in static or non-dev mode?):', err);
  }
